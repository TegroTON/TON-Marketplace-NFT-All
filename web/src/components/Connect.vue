<template>
  <base-button class="order-4 text-sm" secondary @click="toggleModal()">
    <i class="fa-regular fa-arrow-right-to-arc mr-2"></i> Connect
  </base-button>

  <base-modal :open="isModalOpen">
    <div v-if="isConnected" class="modal-content border-0">
      <div class="modal-header d-block border-0 mb-3">
        ...
        <p class="fs-18 color-grey pe-5">Successfully connected</p>
        <button
          aria-label="Close"
          class="border-0 p-0 modal-close"
    ...
    <div v-if="isPickingProvider" class="modal-content border-0">
      <div class="modal-header d-block border-0 mb-3">
        <h5 id="ConnectModalLabel" class="modal-title mb-2 fs-24">
          Connect wallet
        </h5>
        <p class="fs-18 color-grey pe-5">Choose how you want to connect</p>
        <button
          aria-label="Close"
          class="border-0 p-0 modal-close"
          data-bs-dismiss="modal"
      ...
      <div class="modal-body">
        <a
          class="d-flex align-items-center border rounded bg-grey hover-grey p-3"
          type="button"
          @click="connectTonhub()"
        >
    ...
    <div v-if="isConnectingTonhub" class="modal-content border-0">
      <div class="modal-header d-block border-0 mb-3">
        <h5 id="ConnectTonhubModalLabel" class="modal-title mb-2 fs-24">
          Connect Tonhub
        </h5>
      ...
      <div class="modal-body d-block text-center">
        <qrcode-vue
          v-if="tonhubConnectionLink != null"
          :margin="2"
          :size="300"
          :value="tonhubConnectionLink"
          class="mb-2"
        ></qrcode-vue>
...
export default defineComponent({
  name: "Connect",
  components: { BaseModal, BaseButton, QrcodeVue },
  setup() {
  ...
  methods: {
    toggleModal() {
      if (!this.isModalOpen) this.connectionStore.disconnect();

      this.isModalOpen = !this.isModalOpen;
    },
    connectTonhub() {
      this.connectionStore.provider = "tonhub";
      this.tonhubConnectionStore.connect().then(() => {
